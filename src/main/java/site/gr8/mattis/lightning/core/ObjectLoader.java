...
public class ObjectLoader {
	...
	public Model loadOBJModel(String filename) {
		...
		for (Vector3i face : faces) {
			processVertex(face.x, face.y, face.z,
					textures, normals, indices,
					textureCoordsArr, normalsArr);
		}

		int[] indicesArr = indices.stream().mapToInt((Integer v ) -> v).toArray();

		return loadModel(verticesArr, textureCoordsArr, normalsArr, indicesArr);
	}

	private static void processVertex(int pos, int texCoord, int normal,
									  List<Vector2f> texCoordList, List<Vector3f> normalsList, List<Integer> indicesList,
